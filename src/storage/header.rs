...
impl BrainHeader {
    ...
    pub fn deserialize(buf: &[u8]) -> Result<Self> {
        ...
        // Version
        let format_version = u32::from_le_bytes([buf[pos], buf[pos+1], buf[pos+2], buf[pos+3]]);
        if format_version != BRAIN_FORMAT_VERSION {
            return Err(Error::BrainValidation(format!("Unsupported format version: {}", format_version)));
        }
        pos += 4;
        ...
        pos += 8;
        let section_table_count = u32::from_le_bytes([
            buf[pos], buf[pos+1], buf[pos+2], buf[pos+3]
        ]);
        pos += 4;
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.format_version != BRAIN_FORMAT_VERSION {
            return Err(Error::BrainValidation("Unsupported format version".to_string()));
        }
        ...
        if self.cortex_offset > 0 && self.cortex_offset % BRAIN_BLOCK_SIZE as u64 != 0 {
            return Err(Error::BrainValidation("Cortex offset not aligned".to_string()));
        }
        if self.cerebellum_offset > 0 && self.cerebellum_offset % BRAIN_BLOCK_SIZE as u64 != 0 {
            return Err(Error::BrainValidation("Cerebellum offset not aligned".to_string()));
        }
        if self.hippocampus_offset > 0 && self.hippocampus_offset % BRAIN_BLOCK_SIZE as u64 != 0 {
            return Err(Error::BrainValidation("Hippocampus offset not aligned".to_string()));
        }
        
        // Section counts
        if self.section_table_count != 3 {
            return Err(Error::BrainValidation("Invalid section table count".to_string()));
        }
